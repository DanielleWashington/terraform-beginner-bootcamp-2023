...
# we are mocking having a state or database for this development server
...
class Home
  ...
  # want this to only be from cloudfront
  validates :domain_name, 
    format: { with: /\.cloudfront\.net\z/, message: "domain must be from .cloudfront.net" }
    # uniqueness: true, 
  # content version has to be an integer
  # will make sure it is an incremental version in the controller
...
class TerraTownsMockServer < Sinatra::Base
  ...
  # return an a hard coded access token
  def x_access_code
  ...
  def find_user_by_bearer_token
    ...
    # if it is not found or match, then return an error
  ...
  post '/api/u/:user_uuid/homes' do
    ...
    begin
      ...
      # like rails, so it must be manually parsed. 
    ...
    # to see or debug what was inputted into this endpoint. 
    puts "name #{name}"
    ...
    # create a new home model and see the attributes
    home = Home.new
    home.town = town
    home.name = name
    ...
    # ensure that the validation checks pass otherwise
    # return the errors
    unless home.valid?
  ...
  get '/api/u/:user_uuid/homes/:uuid' do
    ...
    content_type :json
    # does the uuid for the home match the one in the mock database
